...
public class BsiBenchmark {
    ...
    public static void setup() throws IOException {
        ...
        for (int i = 0; i < values.size(); i++) {
            bsi.set(i, values.get(i));
        }
        Files.write(new File(BSI_PATH).toPath(), bsi.serialize().array());
        ...
        for (int i = 0; i < 10; i++) {
            ...
            assertThat(bsi.lte(next)).isEqualTo(range.lte(next));
            assertThat(bsi.lte(next, RoaringBitmap.bitmapOfRange(0, ROW_COUNT)))
    ...
    public void testLTE() {
        ...
        for (int value : values) {
            Benchmark benchmark =
                    new Benchmark("bsi-benchmark", read * length)
                            .setNumWarmupIters(1)
